...
function TodosPage() {
  ...
  const addTodo = async (todoTitle) => {
    ...
    try {
      ...
      dispatch({ type: TODO_ACTIONS.ADD_TODO_SUCCESS, payload: { tempId, realTodo } });
    } catch (err) {
      dispatch({
  ...
  const completeTodo = async (id) => {
    ...
    try {
      ...
      dispatch({
        ...
        payload: { id, originalTodo, message: `Status update failed. ${err.message}` },
      });
    }
  };
  ...
  const updateTodo = async (editedTodo) => {
    ...
    try {
      ...
      dispatch({
        ...
        payload: { id: editedTodo.id, originalTodo, message: `Todo edit failed. ${err.message}` },
      });
    }
  };
  ...
  return (
    <div className="todos-page-layout">
      ...
      {filterError && (
        <div className="filter-error-banner" style={{ background: '#fff3cd', padding: '10px', margin: '10px 0' }}>
          <p>{filterError}</p>
      ...
      <SortBy
        ...
        onSortDirectionChange={(newDirection) =>
          dispatch({ type: TODO_ACTIONS.SET_SORT, payload: { sortBy, sortDirection: newDirection } })
        }
      />
      <FilterInput
