...
async function generateCommitMessage() {
  ...
  const touchesDocs = stagedFiles.some(
    (f) => f.endsWith(".md") || f.startsWith("documents/"),
  );
  const touchesEnv = stagedFiles.some(
    (f) =>
      f === "requirements.txt" || f === "package.json" || f.endsWith(".env"),
  );

  if (isWeb && !isPython) {
    const touchesClient = stagedFiles.some(
      (f) => f.includes("client/") || f.includes("components/"),
    );
    const touchesServer = stagedFiles.some(
      (f) => f.includes("server/") || f.includes("api/"),
    );
    ...
    const touchesTools = stagedFiles.some(
      (f) => f.includes("tool") || f.includes("search"),
    );
    const touchesAgent = stagedFiles.some(
      (f) => f.includes("agent") || f.includes("loop") || f.includes("main.py"),
    );
  ...
  try {
    ...
    if (groqKey) {
      try {
        ...
        console.error("❌ Groq fallback failed.");
      }
    }
    ...
    let type = "chore";
    if (stagedFiles.some((f) => f.includes("test") || f.includes("pytest")))
      type = "test";
    else if (
      stagedFiles.some(
        (f) =>
